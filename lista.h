...
/*a menos que en la lista haya un unico nodo y quiera eliminar esa posicion, si se da el caso en el que el siguiente del nodo en el que estoy parada (que podri ser la posicion que quiero elimimar o anterior a la posicion que quiero eliminar) es NULL, ya significa que esa posicion que estoy buscando no existe.
EJEMPLO: tengo el nodo 0 y la posicion que quiero eliminar es justamente la 0, nodo_actual->sig es NULL pero no me importa xq es esa la que quiero eliminar
tengo el nodo 0 1 2 3 y quiero eliminar la posicion 5, una vez que este parada en el nodo 3 (que es el ultimo) y chequee que su siguiente (4) no existe, ya puedo asegurar que la 5 tampoco va a existir y por eso elimino el 3 que es el ultimo.*/